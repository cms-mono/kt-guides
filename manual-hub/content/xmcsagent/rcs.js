/* ============================================================
   매뉴얼 셀 — KT X_McsAgent × KT RCS
   ------------------------------------------------------------
   이 한 건이 "KT X_McsAgent으로 KT RCS 연동하는 절차" 매뉴얼이다.
   실제 내용을 채울 때는 아래 steps(또는 features)를 직접 편집한다.
   - 지금은 공용 더미(HUB_TPL)를 호출하는 상태(= 미작성).
   - 명시적으로 쓰려면 T.socketSteps("RCS", "next_gen") 부분을 steps 배열로 교체:
       steps: [
...
(function () {
  ...
  A["rcs"] = {
    ...
    steps: T.socketSteps("RCS", "next_gen"),  // TCP 소켓 엔진(next_gen 센터) 더미
  };
})();
